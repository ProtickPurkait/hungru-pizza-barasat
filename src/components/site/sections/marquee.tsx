...
export function MarqueeSection({ items }: { items: string[] }) {
  ...
  const Band = ({ reverse, className }: { reverse?: boolean; className: string }) => (
  ...
  return (
    <section className="relative overflow-hidden bg-cream pt-8 pb-16 sm:pt-12 sm:pb-24">
      ...
      <Band reverse className="absolute inset-x-0 top-[46%] -mx-6 rotate-[3.5deg] bg-accent text-ink ring-2 ring-ink sm:top-[44%]" />
      <Band className="relative z-10 -mx-6 -rotate-2 bg-primary text-cream ring-2 ring-ink shadow-[0_10px_0_0_rgb(20_17_16/0.12)]" />
    </section>
  );
}
