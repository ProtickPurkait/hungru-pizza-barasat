...
export function PublishPanel({ hasUnpublishedChanges, history, currentId }: { hasUnpublishedChanges: boolean; history: Version[]; currentId: number | null }) {
      ...
      <Card>
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-sm text-stone-500">{hasUnpublishedChanges ? "Preview them first, then publish when you're happy." : "The live website matches your latest saved content."}</p>
          </div>
          <div className="flex gap-2">
            <a href="/api/preview?path=/" target="_blank" rel="noreferrer" className="inline-flex h-12 items-center gap-2 rounded-lg px-5 font-semibold ring-1 ring-stone-300 hover:bg-stone-50">
              <Eye className="size-4" aria-hidden /> Preview
            </a>
            <Button size="lg" variant="brand" loading={pending} disabled={!hasUnpublishedChanges} onClick={() => run(publishSite)} icon={<Rocket className="size-4" aria-hidden />}>
              Publish now
            </Button>
          </div>
        </div>
              ...
              <li key={v.id} className="flex items-center justify-between gap-3 px-4 py-3 sm:px-5">
                ...
                {v.id !== currentId && (
                  <Button
                    ...
                    onClick={async () => {
                      if (await confirm({ title: `Restore version #${v.id}?`, description: "The live website will go back to how it looked then.", confirmLabel: "Restore", tone: "default" })) {
